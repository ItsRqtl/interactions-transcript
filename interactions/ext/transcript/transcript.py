...
import io
import os
from interactions import Channel, Extension, Guild, MessageType, Message, Sticker, ComponentType
from .utils import Default, get_file_icon, parse_emoji, parse_md, parse_msg_ref, get_file_size, styles
...
async def get_transcript(
    ...
    if mode == "plain":
        ...
        time_format = (
            "%A, %d %B %Y at %H:%M" if military_time else "%A, %d %B %Y at %I:%M %p"
        )
        previous = None
        data = ""
        metadata = {}
        for i in msg:
            ...
            edit = (i.edited_timestamp.astimezone(pytz.timezone(pytz_timezone)).strftime(
                    time_format
                )
                if i.edited_timestamp
                else None
            ...
            if i.type == MessageType.CHANNEL_PINNED_MESSAGE:
                ...
                rawhtml = rawhtml.replace(
                    "{{USER_COLOUR}}",
                    await parse_md(f"color: {hex(i.author.accent_color)[2:] if i.author.accent_color else '000000'}", channel),
                )
                rawhtml = rawhtml.replace(
                    "{{NAME}}", await parse_md(str(html.escape(i.author.username)), channel)
                )
                rawhtml = rawhtml.replace(
                ...
                if i.content:
                    ...
                    rawhtml = rawhtml.replace(
                        "{{MESSAGE_CONTENT}}",
                        await parse_md(str(html.escape(i.content)), channel),
                    )
                    rawhtml = rawhtml.replace(
                        "{{EDIT}}",
                ...
                if not i.referenced_message:
                    ...
                    if not (
                        ...
                        rawhtml = rawhtml.replace(
                            "{{NAME}}",
                            await parse_md(str(html.escape(ref.author.username)), channel),
                        )
                        rawhtml = rawhtml.replace(
                            "{{NAME_TAG}}",
                        ...
                        rawhtml = rawhtml.replace(
                            "{{CONTENT}}", await parse_msg_ref(ref.content, channel)
                        )
                        rawhtml = rawhtml.replace(
                            "{{EDIT}}",
                            f'<span class="chatlog__reference-edited-timestamp" title="{i.edited_timestamp}">(edited)</span>'
                ...
                if i.sticker_items:
                    if i.sticker_items[0].format_type == 3:
                        sticker = Sticker(**await channel._client.get_sticker(i.sticker_items[0].id))
                ...
                if i.attachments:
                    for a in i.attachments:
                        if a.content_type is None or ('image' not in a.content_type and 'video' not in a.content_type and 'audio' not in a.content_type):
                            ...
                            rawhtml = rawhtml.replace("{{ATTACH_BYTES}}", str(get_file_size(a.size)))
                            ...
                            if "image" in a.content_type:
                                ...
                                with open(dir_path + "/html/attachment/audio.html", "r") as f:
                                    rawhtml = f.read()
                                rawhtml = rawhtml.replace("{{ATTACH_ICON}}", Default.file_attachment_audio)
                                rawhtml = rawhtml.replace("{{ATTACH_URL}}", str(a.url))
                                rawhtml = rawhtml.replace("{{ATTACH_BYTES}}", str(get_file_size(a.size)))
                    ...
                    for r in i.components:
                        for c in r.components:
                            if c.type == ComponentType.BUTTON:
                                with open(dir_path+"/html/component/component_button.html", "r") as f:
                                    rawhtml = f.read()
                                rawhtml = rawhtml.replace("{{DISABLED}}", "chatlog__component-disabled" if c.disabled else "")
                                rawhtml = rawhtml.replace("{{URL}}", c.url if c.url else "")
                                rawhtml = rawhtml.replace("{{LABEL}}", await parse_md(c.label if c.label else "", channel))
                                rawhtml = rawhtml.replace("{{EMOJI}}", await parse_emoji(str(c.emoji) if c.emoji else "", channel))
                                rawhtml = rawhtml.replace("{{ICON}}", Default.button_external_link if c.url else "")
                                ...
                                if not c.disabled:
                                    ...
                                    for option in c.options:
                                        if option.emoji:
                                            with open(dir_path+"/html/component/component_menu_option.html", "r") as f:
                                                rawhtml = f.read()
                                            rawhtml = rawhtml.replace("{{EMOJI}}", await parse_emoji(str(option.emoji), channel))
                                            rawhtml = rawhtml.replace("{{TITLE}}", await parse_md(str(option.label), channel))
                                            rawhtml = rawhtml.replace("{{DESCRIPTION}}", await parse_md(str(option.description) if option.description else "", channel))
                                        else:
                                            with open(dir_path+"/html/component/component_menu_option_emoji.html", "r") as f:
                                                rawhtml = f.read()
                                            rawhtml = rawhtml.replace("{{TITLE}}", await parse_md(str(option.label), channel))
                                            rawhtml = rawhtml.replace("{{DESCRIPTION}}", await parse_md(str(option.description) if option.description else "", channel))
                                ...
                                with open(dir_path+"/html/component/component_menu.html", "r") as f:
                                    rawhtml = f.read()
                                rawhtml = rawhtml.replace("{{DISABLED}}", "chatlog__component-disabled" if c.disabled else "")
                                rawhtml = rawhtml.replace("{{PLACEHOLDER}}", await parse_md(c.placeholder if c.placeholder else "", channel))
                                rawhtml = rawhtml.replace("{{ID}}", str(menu_div_id))
                                rawhtml = rawhtml.replace("{{CONTENT}}", str(option_content))
                                rawhtml = rawhtml.replace("{{ICON}}", Default.interaction_dropdown_icon)
                                components += f'<div class="chatlog__components">{rawhtml}</div>'
                                menu_div_id += 1
                ...
                if i.reactions:
                    for r in i.reactions:
                        if not r.emoji.id:
                            ...
                            rawhtml = rawhtml.replace("{{EMOJI_FILE}}", 'gif' if r.emoji.animated else 'png')
                        else:
                ...
                if (
                    ...
                    rawhtml = rawhtml.replace(
                        "{{USER_ID}}", await parse_md(str(i.author.id), channel)
                    )
                    rawhtml = rawhtml.replace(
                        "{{USER_COLOUR}}",
                        await parse_md(
                            f"color: {hex(i.author.accent_color)[2:] if i.author.accent_color else '000000'}", channel
                        ),
                    )
                    ...
                    rawhtml = rawhtml.replace("{{MESSAGE_ID}}", await parse_md(str(i.id), channel))
                ...
                if str(user_id:=i.author.id) in metadata:
                    ...
                    bot = i.author.bot
                    user_avatar = i.author.avatar_url
                    metadata[str(user_id)] = [username, created_at, bot, user_avatar, 1]
        ...
        meta_data_html: str = ""
        for md in metadata:
            creation_time = metadata[str(md)][1].astimezone(pytz.timezone(pytz_timezone)).strftime("%d/%m/%y @ %T")
        ...
        channel_topic = f'<span class="panel__channel-topic">{channel.topic}</span>' if channel.topic else ""
        ...
        with open(dir_path + "/html/base.html", "r") as f:
            rawhtml = f.read()
        rawhtml = rawhtml.replace("{{SERVER_NAME}}", await parse_md(f"{html.escape(guild.name)}", channel))
        rawhtml = rawhtml.replace("{{SERVER_AVATAR_URL}}", str(guild.icon_url if guild.icon_url else Default.default_avatar))
        rawhtml = rawhtml.replace("{{CHANNEL_NAME}}", await parse_md(f"{channel.name}", channel))
        ...
        rawhtml = rawhtml.replace("{{TIMEZONE}}", str(pytz_timezone))
        rawhtml = rawhtml.replace("{{DATE_TIME}}", str(datetime.now(pytz.timezone(pytz_timezone)).strftime("%e %B %Y at %T (%Z)")))
        rawhtml = rawhtml.replace("{{CHANNEL_CREATED_AT}}", str(channel.id.timestamp.astimezone(pytz.timezone(pytz_timezone)).strftime("%d/%m/%y @ %T")))
