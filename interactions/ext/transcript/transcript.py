    ...
    if mode == 'plain':
        ...
        for i in msg:
            if military_time:
                ...
                time = i.id.timestamp.astimezone(pytz.timezone(pytz_timezone)).strftime('%d-%b-%y %I:%M:%S%p')
                edit_time = i.edited_timestamp.astimezone(pytz.timezone(pytz_timezone)).strftime('%d-%b-%y %I:%M:%S%p') if i.edited_timestamp else None
            data.append({"Guild": {"name": guild.name, "id": str(guild.id)}, "Channel": {"name": channel.name, "id": str(channel.id)}, "Author": {"username": i.author.username+"#"+i.author.discriminator, "id": str(i.author.id)}, "Time": time, "Edited": edit_time, "Content": i.content, "Embeds": [{"title": e.title, "description": e.description, "author": {"name": e.author.name, "url": e.author.url, "icon": e.author.icon_url} if e.author else {}, "thumbnail": e.thumbnail.url if e.thumbnail else None, "image": e.image.url if e.image else None, "fields": [{"name": f.name, "value": f.value, "inline": f.inline} for f in e.fields] if e.fields else []} for e in i.embeds] if i.embeds else [], "Attachments": [a.url for a in i.attachments] if i.attachments else [], "Reactions": [{r.emoji:r.count} for r in i.reactions] if i.reactions else []})
        df = pd.DataFrame(data)
        if mode == 'csv':
            df.to_csv(file:=io.StringIO(), index=True, header=True)
            return file.getvalue()
        elif mode == 'xml':
            df.to_xml(file:=io.BytesIO(), index=True, encoding='utf-8', root_name='transcript', row_name='message')
